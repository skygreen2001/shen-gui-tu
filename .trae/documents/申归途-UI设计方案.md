...
**设计理念**：信息密集但有序。4个Tab分类，数据驱动，易于更新。资源数据集中在 `src/data/` 目录，修改数据文件即可更新内容。
...
#### 轮次 6：Resources 上海资源页

**交付物**：`prototype-06-resources.html`
- 包含：4个Tab切换、医院列表（含筛选）、热线列表、社区资源、医保政策
...
| 离线运行 | HashRouter、无CDN资源、LocalStorage持久化 |
| 移动端优先 | 480px max-width、44px最小触摸目标、大字体大按钮 |
| 易于更新资源 | 数据与视图分离，`src/data/` 目录独立管理 |
