...
export type MedicationDB = Record<string, MedicationInfo>;

export const medicationDB: MedicationDB = {
  '草酸艾司西酞普兰': {
...
export const defaultMissedAdvice: string = '偶尔漏服一次通常不会造成严重影响。发现后尽快补服即可，但不要一次服用双倍剂量。如果经常漏服，建议和医生讨论是否有更方便的用药方案。'

export function getMedInfo(name: string): MedicationInfo | null {
  return medicationDB[name] || null
}

export function getMissedDoseAdvice(name: string): string {
